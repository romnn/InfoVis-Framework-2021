...
attrs = {
    "living_space": ["WOPP4060_P","WOPP6080_P","WOPP80100_P","WOPP100PLUS_P"],
    "rent": ["WHUURTSLG_P","WHUURMIDDEN_P","WHUURHOOG_P"],
    "prop": ["WCORHUUR_P","WPARTHUUR_P","WKOOP_P"],
}
...
def d3():
        ...
        return render_template("d3.html", meta_data=meta_data,
                x_variables=data.model_vars, area_names=data.area_names, attr_names=attrs.keys(), selected_area_name=area_name, selected_attr_name=attr_name)

@main.route('/d3_plot_data', methods = ['GET'])
def d3_plot_data_living_space():
        area_name = request.args.get("area_name")
        typ = request.args.get("type")
        plot_data = data.stats_ams.loc[data.stats_ams['area_name'] == area_name]
        plot_data = plot_data.loc[:, plot_data.columns.isin(attrs[typ])]
        plot_data = plot_data.to_json(orient='records')
        return plot_data
